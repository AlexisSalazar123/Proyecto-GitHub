<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sel = $con->query("SELECT * FROM usuario where usuario='$usuario'");
        ...
        if($sel) {
            if($sel->num_rows > 0) {
                ...
                if ($contraseña_bd == $contraseña_ingresada) {
                    if($registro['rol']==1){
                        header("Location: ../index.php"); //Este sera el index que tendra acceso a todo
                    }
                    elseif($registro['rol']==2){
                        header("Location: ../index.php"); //Este sera el index que tendra acceso a unas cuantas cosas
                    }
                } else {
